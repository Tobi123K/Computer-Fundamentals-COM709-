...
class NewsLetterGui (Tk):
    ...
    def add_email_frame(self):
        ...
        self.email_entry.configure(bg="white",
                                   width=30)
        self.email_entry.bind("<KeyRelease>", self.not_entry_email)

        
    def not_entry_email(self,event):
        if (self.email_entry.get() != ""):
            self.defult_image_label.configure(image=self.filled_image,
                                            padx = 10)
        else:
            self.defult_image_label.configure(image=self.empty_image,
                                            padx = 10)
    ...
    def submit_button_clicked(self, event):
        ...
        messagebox.showinfo("NewsLetterGui","Subscribed!")
